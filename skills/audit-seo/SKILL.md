---
name: audit-seo
description: Perform technical and content SEO audits of web projects from source code and optional live URLs. Use for metadata, Open Graph, robots.txt, sitemap, canonical, hreflang, structured data, Core Web Vitals, internal linking, AI-search/AEO, and framework-specific SEO reviews.
---
...
## 1bis. Boundaries - périmètre de l'audit

Ce skill audite des projets web **génériques** (Next.js, Nuxt, Astro, SvelteKit, Remix, Laravel, Symfony, WordPress, statique, SPA) à partir de leur code source. Il couvre le SEO **technique, on-page, structurel et de contenu**.

Hors périmètre (à confier à un outil dédié si besoin) :

- Pages de marketplace / store listing à rendu automatique par une plateforme tierce (sections produit générées, descriptions imposées par la plateforme).
- Pricing / monetization d'un produit.
- Ingénierie de scraping / anti-bot.
- Off-page SEO et link building (cet audit reste on-page et technique).

Pour les concerns AI-search / AEO / GEO **génériques** (LLM crawlers, `llms.txt`, citations dans AI Overview), voir `checklists/ai-search-seo.md`.
